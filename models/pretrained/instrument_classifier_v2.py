...
- Reusing existing embeddings and models from saved_data to reduce computation
...
def load_dataset(data_path, test_size=0.2, random_state=42, save_path=None, original_save_path=None):
    """
    ...
    if original_save_path is not None and os.path.exists(os.path.join(original_save_path, 'dataset_split.pkl')):
        print(f"Loading existing dataset split from {os.path.join(original_save_path, 'dataset_split.pkl')}")
        with open(os.path.join(original_save_path, 'dataset_split.pkl'), 'rb') as f:
            ...
            if save_path is not None and save_path != original_save_path:
                ...
                with open(os.path.join(save_path, 'dataset_split.pkl'), 'wb') as f_new:
                    pickle.dump(dataset, f_new)
                print(f"Dataset split copied to {os.path.join(save_path, 'dataset_split.pkl')}")
...
def extract_embeddings(audio_files, yamnet_model, save_path=None, load_from_disk=False, original_save_path=None):
    """
    ...
    if load_from_disk and original_save_path is not None and os.path.exists(os.path.join(original_save_path, 'yamnet_embeddings.pkl')):
        print(f"Checking for embeddings in original location: {os.path.join(original_save_path, 'yamnet_embeddings.pkl')}")
        with open(os.path.join(original_save_path, 'yamnet_embeddings.pkl'), 'rb') as f:
            data = pickle.load(f)
            ...
            if all(file in successful_files for file in audio_files):
                ...
                if save_path is not None and save_path != original_save_path:
                    ...
                    with open(os.path.join(save_path, 'yamnet_embeddings.pkl'), 'wb') as f_new:
                        pickle.dump(new_data, f_new)
                    print(f"Embeddings copied to {os.path.join(save_path, 'yamnet_embeddings.pkl')}")
                
                return filtered_embeddings, audio_files
            else:
                print("Not all requested files found in original saved embeddings.")
    
    # Then check in the new directory
    ...
    # If we reach here, we need to extract embeddings from scratch
...
def train_classifiers(X_train_yamnet, y_train, save_path=None, original_save_path=None):
    """
    Train classifiers on the embeddings or load from existing models.
    
    Args:
        X_train_yamnet (np.ndarray): YAMNet embeddings for training.
        y_train (np.ndarray): Training labels.
        save_path (str, optional): Path to save models to disk.
        original_save_path (str, optional): Path to check for existing trained models.
        
    Returns:
        yamnet_models (dict): Trained models.
    """
    # First check if models exist in the original directory
    if original_save_path is not None and os.path.exists(os.path.join(original_save_path, 'trained_models.pkl')):
        print(f"Loading existing models from {os.path.join(original_save_path, 'trained_models.pkl')}")
        with open(os.path.join(original_save_path, 'trained_models.pkl'), 'rb') as f:
            yamnet_models = pickle.load(f)
            
            # Copy to new location if different
            if save_path is not None and save_path != original_save_path:
                os.makedirs(save_path, exist_ok=True)
                with open(os.path.join(save_path, 'trained_models.pkl'), 'wb') as f_new:
                    pickle.dump(yamnet_models, f_new)
                print(f"Models copied to {os.path.join(save_path, 'trained_models.pkl')}")
...
def evaluate_classifiers(models, X_test, y_test, label_encoder, save_path=None):
    ...
    for name, model in models.items():
        ...
        # Create a dictionary mapping class names to their metrics
        class_metrics = {}
        ...
        if save_path is not None:
            ...
            metrics_json_path = os.path.join(save_path, f'{name.replace(" ", "_")}_metrics.json')
            with open(metrics_json_path, 'w') as f:
                json.dump(class_metrics, f, indent=4)
            print(f"Class metrics saved to {metrics_json_path}")
            
            # Save metrics as CSV
            metrics_csv_path = os.path.join(save_path, f'{name.replace(" ", "_")}_metrics.csv')
            metrics_df.to_csv(metrics_csv_path)
            print(f"Class metrics saved to {metrics_csv_path}")
...
def main():
    print("Instrument Classifier v2 using YAMNet embeddings (with per-class metrics)")
    print("-" * 75)
    
    # Define directories
    new_output_dir = 'models/pretrained/saved_data_v2'
    original_output_dir = 'models/pretrained/saved_data'  # Original directory with precomputed data
    os.makedirs(new_output_dir, exist_ok=True)
    
    print(f"Using precomputed data from {original_output_dir} when available")
    print(f"Saving new results to {new_output_dir}")
    
    # Load dataset
    data_path = 'data/instruments'
    X_train, X_test, y_train, y_test, label_encoder = load_dataset(
        data_path, 
        save_path=new_output_dir,
        original_save_path=original_output_dir
    )
    ...
    # Extract embeddings with saving to disk - try to reuse from original directory
    ...
    # Train classifiers - try to reuse from original directory
    yamnet_models = train_classifiers(
        X_train_yamnet, y_train_filtered, 
        save_path=new_output_dir,
        original_save_path=original_output_dir
    )
    ...
    results = evaluate_classifiers(
        yamnet_models, X_test_yamnet, y_test_filtered, label_encoder, save_path=new_output_dir
    )
    
    # Display results
    display_results(results, save_path=new_output_dir)
    
    # Create a consolidated metrics file
    consolidated_metrics = {}
    for model_name, model_results in results.items():
        consolidated_metrics[model_name] = model_results['class_metrics']
    
    # Save consolidated metrics
    with open(os.path.join(new_output_dir, 'metrics.json'), 'w') as f:
        json.dump(consolidated_metrics, f, indent=4)
    
    # Also save as CSV (flattened structure for easier reading)
    metrics_data = []
    for model_name, class_metrics in consolidated_metrics.items():
    ...
    metrics_df.to_csv(os.path.join(new_output_dir, 'metrics.csv'), index=False)
    
    print(f"\nAll data and models saved to {os.path.abspath(new_output_dir)}")
    print(f"Per-class precision, recall, and F1 scores saved to {os.path.join(new_output_dir, 'metrics.json')} and {os.path.join(new_output_dir, 'metrics.csv')}")
    print(f"\nNote: Computation was reduced by reusing precomputed data from {original_output_dir} where available")
